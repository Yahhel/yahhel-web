...
import { Check, Download, Eye, Lock, ShoppingBag } from "lucide-react";
...
export function ProductCard({
  ...
  return (
    <article className={cn("relative box-border rounded-[20px] bg-white px-6 pb-5 pt-6 font-sans text-[#1a1a1a] shadow-[0_1px_2px_rgba(20,20,20,.04),0_6px_20px_rgba(20,20,20,.06)]",
      ...
      <button
        ...
        className="absolute right-6 top-7 cursor-pointer p-1 leading-none text-[#6b6560]"
      >
        <svg
          className="h-4 w-4"
          viewBox="0 0 24 24"
      ...
      <div className="flex gap-6 pb-5 pl-10">
        ...
        <div className="min-w-0 flex-1 pr-7">
          ...
          <div className="flex items-center gap-3">
            <span
              className={cn(`rounded-full px-3 py-1.25 text-[10px] font-medium`,
            ...
            <span className="text-xs text-[#1D1816] font-semibold">{naira(p.price)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-4">
        <div>
          <div className="mb-1 text-[10px] text-[#766860]">Revenue · Conv</div>
        ...
        <div className="text-right text-sm leading-normal text-[#7a736c]">
          ...
          <p className="text-[10px] text-[#766860]">{formatDate(p.publishedAt)}</p>
        </div>
      </div>
    </article>
  );
